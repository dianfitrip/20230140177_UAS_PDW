...
<?php if ($message): ?>
<div class="mb-4 px-4 py-3 rounded-lg <?php echo ($message_type == 'success') ? 'bg-emerald-100 border border-emerald-400 text-emerald-700' : 'bg-rose-100 border border-rose-400 text-rose-700'; ?>" role="alert">
...
<div class="bg-white p-6 rounded-lg shadow-md mb-8">
    ...
    <form action="matkul.php" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nama_praktikum" id="nama_praktikum" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:ring-purple-500 focus:border-purple-500" value="<?php echo htmlspecialchars($matkul_to_edit['nama_praktikum'] ?? ''); ?>" required>
        </div>
        
        <div class="mb-4">
            <label for="deskripsi" class="block text-gray-700 text-sm font-bold mb-2">Deskripsi Singkat:</label>
            <textarea name="deskripsi" id="deskripsi" rows="3" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:ring-purple-500 focus:border-purple-500"><?php echo htmlspecialchars($matkul_to_edit['deskripsi'] ?? ''); ?></textarea>
        </div>
        
        <div class="flex items-center justify-between">
            <button type="submit" name="submit" class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition-colors duration-300">
                Simpan Data
            </button>
            <?php if ($matkul_to_edit): // Tampilkan tombol Batal hanya saat mode edit ?>
            <a href="matkul.php" class="inline-block align-baseline font-bold text-sm text-indigo-600 hover:text-indigo-800 transition-colors duration-300">
                Batal Edit
            </a>
            <?php endif; ?>
        </div>
        ...
        <tbody class="text-gray-700">
            ...
            <tr class="border-b hover:bg-purple-50">
                ...
                <td class="py-3 px-4 whitespace-nowrap">
                    <a href="matkul.php?edit=<?php echo $row['id']; ?>" class="text-blue-600 hover:text-blue-800 font-semibold mr-4 transition-colors duration-300">Edit</a>
                    <a href="matkul.php?delete=<?php echo $row['id']; ?>" class="text-red-600 hover:text-red-800 font-semibold transition-colors duration-300" onclick="return confirm('Apakah Anda yakin ingin menghapus mata praktikum ini? Semua modul terkait juga akan terhapus.');">Hapus</a>
                </td>
            </tr>
            <?php
                endwhile;
