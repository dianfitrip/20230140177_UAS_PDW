...
<?php if ($message): ?>
<div class="mb-4 px-4 py-3 rounded-lg <?php echo ($message_type == 'success') ? 'bg-emerald-100 border-emerald-400 text-emerald-700' : 'bg-rose-100 border-rose-400 text-rose-700'; ?>" role="alert">
...
<div class="bg-white p-6 rounded-lg shadow-md mb-8">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="laporan.php" class="text-purple-600 hover:underline">&larr; Kembali ke Daftar Laporan</a>
    </div>
    <div class="mb-4 border-b pb-4">
        ...
        <p><strong>File Laporan:</strong> 
            <a href="../uploads/<?php echo htmlspecialchars($laporan_to_grade['file_laporan']); ?>" target="_blank" class="text-indigo-600 hover:underline">
                Download Laporan
            </a>
        </p>
    </div>
    <form action="laporan.php" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="number" name="nilai" id="nilai" min="0" max="100" class="shadow border rounded w-full py-2 px-3 text-gray-700 focus:ring-purple-500 focus:border-purple-500" value="<?php echo $laporan_to_grade['nilai'] ?? ''; ?>" required>
        </div>
        <div class="mb-4">
            <label for="feedback" class="block text-gray-700 text-sm font-bold mb-2">Feedback (Opsional):</label>
            <textarea name="feedback" id="feedback" rows="4" class="shadow border rounded w-full py-2 px-3 text-gray-700 focus:ring-purple-500 focus:border-purple-500"><?php echo htmlspecialchars($laporan_to_grade['feedback'] ?? ''); ?></textarea>
        </div>
        <button type="submit" name="submit_nilai" class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded transition-colors duration-300">Simpan Nilai</button>
    </form>
</div>
...
<div class="bg-white p-4 rounded-lg shadow-md mb-6">
    <form action="laporan.php" method="GET" class="flex flex-col md:flex-row md:items-end gap-4">
        <div>
            ...
            <select name="filter_praktikum" id="filter_praktikum" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-purple-500 focus:border-purple-500 sm:text-sm rounded-md">
        ...
        <div>
            ...
            <select name="filter_status" id="filter_status" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-purple-500 focus:border-purple-500 sm:text-sm rounded-md">
        ...
        <div class="flex items-center gap-2">
            <button type="submit" class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-md transition-colors duration-300">Filter</button>
            <a href="laporan.php" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-md transition-colors duration-300">Reset</a>
        </div>
    </form>
</div>
    ...
    <table class="min-w-full bg-white">
        ...
        <tbody class="text-gray-700">
            ...
            <tr class="border-b hover:bg-purple-50">
                ...
                <td class="py-3 px-4">
                    <span class="px-2 py-1 font-semibold leading-tight rounded-full <?php echo ($row['status'] == 'Dinilai') ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'; ?>">
                        <?php echo $row['status']; ?>
                    </span>
                </td>
                <td class="py-3 px-4 font-bold"><?php echo $row['nilai'] ?? '-'; ?></td>
                <td class="py-3 px-4">
                    <a href="laporan.php?nilai=<?php echo $row['id']; ?>" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold py-1 px-3 rounded transition-colors duration-300">
