...
$activePage = 'my_courses'; // Assuming this is for student dashboard, adjust if needed
require_once 'templates/header_mahasiswa.php'; // Make sure this header exists and has the new theme
?>

<?php if ($message): ?>
<div class="mb-6 px-4 py-3 rounded-lg <?php echo ($message_type == 'success') ? 'bg-emerald-100 border-emerald-400 text-emerald-700' : 'bg-rose-100 border-rose-400 text-rose-700'; ?>" role="alert">
...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="my_courses.php" class="text-purple-600 hover:underline">&larr; Kembali ke Praktikum Saya</a>
</div>
...
<div class="space-y-6">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <div class="flex flex-col md:flex-row justify-between items-start">
            <div class="mb-4 md:mb-0">
                ...
                <a href="../uploads/<?php echo htmlspecialchars($modul['file_materi']); ?>" target="_blank" class="text-indigo-600 hover:underline inline-block mt-2">
                    Download Materi
                </a>
                <?php else: ?>
            ...
            <div class="w-full md:w-1/2">
                <form action="course_detail.php?id=<?php echo $praktikum_id; ?>" method="POST" enctype="multipart/form-data">
                    ...
                    <label for="file_laporan_<?php echo $modul['id']; ?>" class="block text-sm font-medium text-gray-700 mb-1">Kumpulkan Laporan (PDF, DOCX, ZIP, RAR):</label>
                    <div class="flex items-center">
                        <input type="file" name="file_laporan" id="file_laporan_<?php echo $modul['id']; ?>" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100" required>
                        <button type="submit" name="kumpul_laporan" class="ml-4 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors duration-300">Upload</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="border-t border-gray-200 mt-4 pt-4">
            ...
            <?php if ($laporan): ?>
                <div class="mt-2 p-3 rounded-lg bg-purple-50">
                    <p><strong>File Terkumpul:</strong> <a href="../uploads/<?php echo htmlspecialchars($laporan['file_laporan']); ?>" target="_blank" class="text-indigo-600 hover:underline"><?php echo htmlspecialchars($laporan['file_laporan']); ?></a></p>
                    <p><strong>Waktu Kumpul:</strong> <?php echo date('d M Y, H:i', strtotime($laporan['tanggal_kumpul'])); ?></p>
                    <p><strong>Status:</strong> 
                        <span class="font-bold <?php echo ($laporan['status'] == 'Dinilai') ? 'text-emerald-600' : 'text-amber-600'; ?>">
                            <?php echo $laporan['status']; ?>
                        </span>
                    </p>
                    <?php if ($laporan['status'] == 'Dinilai'): ?>
                    <p><strong>Nilai:</strong> <span class="text-2xl font-bold text-indigo-700"><?php echo $laporan['nilai']; ?></span></p>
    ...
    $stmt_modul->close();
    // Removed the problematic $conn->close(); from here
    ?>
</div>
